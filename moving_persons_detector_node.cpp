...
private:
    ...
    geometry_msgs::Point person_coordinates;
...
//else we start a new cluster with the current hit and end the current cluster
    ...
    for (int loop=1; loop<nb_beams; loop++)//loop over all the hits
        if (distancePoints(current_scan[loop], current_scan[loop-1]) < cluster_threshold) {
            ...
            cluster_dynamic[nb_cluster] = nb_dynamic * 100 / (cluster_end[nb_cluster] + 1 - cluster_start[nb_cluster]);
            cluster_size[nb_cluster] = distancePoints(current_scan[cluster_end[nb_cluster]], current_scan[cluster_start[nb_cluster]]);
            geometry_msgs::Point new_middle;
            new_middle.x = (current_scan[cluster_end[nb_cluster]].x + current_scan[cluster_start[nb_cluster]].x) / 2;
            new_middle.y = (current_scan[cluster_end[nb_cluster]].y + current_scan[cluster_start[nb_cluster]].y) / 2;
            new_middle.z = 0.0;
            cluster_middle[nb_cluster] = new_middle;

            //textual display
            ROS_INFO("cluster[%i]: (%f, %f) -> (%f, %f), size: %f, dynamic: %i", nb_cluster, current_scan[cluster_start[nb_cluster]].x, current_scan[cluster_start[nb_cluster]].y, current_scan[cluster_end[nb_cluster]].x, current_scan[cluster_end[nb_cluster]].y,
...
// a moving person has two moving legs located at less than "legs_distance_max" one from the other
    ...
    for (int loop_leg1=0; loop_leg1<nb_moving_legs_detected; loop_leg1++) {//loop over all the legs
        for (int loop_leg2=loop_leg1+1; loop_leg2<nb_moving_legs_detected; loop_leg2++) {//loop over all the legs
            if (distancePoints(moving_leg_detected[loop_leg1], moving_leg_detected[loop_leg2]) < legs_distance_max) {// then we find a moving person

                // we update the moving_persons_detected table to store the middle of the moving person
                geometry_msgs::Point new_middle;
                new_middle.x = (moving_leg_detected[cluster[loop_leg1]].x + moving_leg_detected[cluster[loop_leg2]].x) / 2;
                new_middle.y = (moving_leg_detected[cluster[loop_leg1]].y + moving_leg_detected[cluster[loop_leg2]].y) / 2;
                new_middle.z = 0.0;
                moving_persons_detected[nb_moving_persons_detected] = new_middle;
                ...
                goal_to_reach.z = moving_persons_detected[nb_moving_persons_detected].z;
            }
        }
    }
